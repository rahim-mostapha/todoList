...
let LoginComponent = class LoginComponent {
    constructor(fb, flashMessage, router, route, user) {
        this.fb = fb;
...
let NewTaskComponent = class NewTaskComponent {
    constructor(fb, task, flashMessage, route) {
        this.fb = fb;
        this.task = task;
        this.flashMessage = flashMessage;
        this.route = route;
    ...
    getUserData() {
        this.task.getTaskData(this.pageCase['id']).subscribe((res) => {
            if (res['status'] === "done") {
                let data = res['data'];
    ...
    onSubmit() {
        ...
        this.task.newAndUpdateTask(data, this.pageCase['case']).subscribe((res) => {
            if (res['status'] === "done") {
...
let SignupComponent = class SignupComponent {
    constructor(fb, router, user, flashMessage, route) {
        this.fb = fb;
        this.router = router;
        this.user = user;
        this.flashMessage = flashMessage;
    ...
    onSubmit() {
        this.user.loginOrSignupOrUpdateUser(this.signupForm.value, this.pageCase).subscribe((res) => {
            if (res['status'] === "done") {
                this.flashMessage.show(`${res['status']} : you can Login now `, { cssClass: "alert-success" });
...
let TasksComponent = class TasksComponent {
    constructor(task, flashMessage) {
        this.task = task;
        this.flashMessage = flashMessage;
    }
    ngOnInit() {
        this.task.listAllTasks().subscribe(res => {
            if (res['status'] === 'done') {
                this.tasks = res['data'];
            }
            else {
    ...
    removeTask(_id) {
        this.task.deleteTask(_id).subscribe(res => {
            if (res['status'] === 'done') {
